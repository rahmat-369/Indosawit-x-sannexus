...
import { Instagram, Send, MessageCircle, PlusCircle, Trash2, Copy, RefreshCw, Edit2, Share2, ChevronUp, ChevronDown, HelpCircle, Globe, Link as LinkIcon, Zap } from 'lucide-react';
...
// RENDERER MARKDOWN DENGAN SITASI SUPERSCRIPT (GPT STYLE)
const formatMarkdown = (text) => {
  ...
  // Ubah [1] jadi pangkat interaktif
...
export default function SanexusChat({ initialQuery, onClose }) {
  ...
  const [showInfo, setShowInfo] = useState(false); // Popover info model
  ...
  // Animasi Loading Humanoid
  useEffect(() => {
    let timer;
    if (isLoading) {
      const texts = [
        "Menganalisis memori obrolan...", 
        "Mencari data terbaru...", 
        "Membaca sumber artikel...", 
        "Menyusun rangkuman...", 
      ...
      const updateLoadingText = () => {
        ...
        step++;
        // Kecepatan acak antara 800ms sampai 2500ms
        timer = setTimeout(updateLoadingText, Math.floor(Math.random() * 1700) + 800);
      };
  ...
  // LOGIKA PENCARIAN & MEMORI
  const performSearch = async (queryText, forceModel = null) => {
      if (!queryText.trim()) return;

      // 1. Injeksi Memori: Ambil konteks 2 pesan terakhir biar AI gak amnesia
      let contextQuery = queryText;
      if (messages.length > 0) {
        const history = messages.slice(-2).map(m => `${m.role === 'user' ? 'User' : 'AI'}: ${m.content || 'Share Berita'}`).join("\n");
        contextQuery = `HISTORY OBROLAN SINGKAT:\n${history}\n\nPERTANYAAN BARU USER: ${queryText}`;
      }
      ...
      // 2. AUTO MODEL LOGIC (Deteksi Intent)
      const needsInternet = /berita|cari|internet|deep|detil|analisis|kabarnya|tadi|siapa|info lainnya|lanjut/i.test(queryText);
      ...
      try {
        ...
        // Fallback jika API Deep error, oper ke Fast
        if (targetModel === 'deep') {
           try {
  ...
  const handleSubmit = (e) => {
    ...
    if (pendingNews) {
      ...
      // Paksa mode Deep untuk baca berita
      performSearch(`Analisis berita: ${pendingNews.title}. Link: ${pendingNews.link}. Instruksi User: ${customText}`, 'deep');
    } else {
  ...
  return (
    <div className="sanexus-wrapper">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* SOURCE CARDS IDENTITY (BANG SAN STYLE) */
      ...
      <main className="s-chat-area">
        ...
        {isLoading && (
          <div style={{textAlign: 'center', padding: '30px 0', opacity: 0.8}}>
            ...
            <p className="text-[10px] uppercase tracking-widest text-[#b8cbb8] font-bold mt-4 transition-all duration-300">{loadingText}</p>
          </div>
        )}
      ...
      <div className="s-input-container">
        ...
        {/* PREVIEW KARTU BERITA SEBELUM KIRIM */}
        {pendingNews && (
        ...
        <form onSubmit={handleSubmit} className="s-search-bar">
          <input 
            ...
            disabled={false} /* Input tetap nyala biar user bisa nyicil ngetik */
            required={!pendingNews}
          />
          {/* TOMBOL KIRIM (MATI SAAT LOADING) */}
