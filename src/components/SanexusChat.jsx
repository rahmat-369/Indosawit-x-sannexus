...
// Custom SVG Icons untuk Full Page Profile
...
export default function SanexusChat({ initialQuery = "", onClose }) {
  ...
  const [showFullProfile, setShowFullProfile] = useState(false); // State Modal Profil Full
  ...
  return (
    <div className="sanexus-wrapper">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* MINI PROFILE BAR (DI BAWAH SIDEBAR) */
        ...
        .s-full-profile-overlay { position: fixed; inset: 0; background: var(--bg-dark); z-index: 300; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; text-align: center; animation: profileFade 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); }
        @keyframes profileFade { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: scale(1); } }
        
        .s-close-full-profile { position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.1); border: none; color: #fff; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; }
        ...
        /* CLASS ANIMASI HALUS MUNCULNYA AI */
      ...
      {/* SIDEBAR DENGAN HISTORI TRUNCATE & MINI PROFILE BAWAH */}
      <aside className={`s-sidebar ${isSidebarOpen ? 'visible' : ''}`} onClick={(e) => {if(e.target === e.currentTarget) setIsSidebarOpen(false)}}>
        <div className="s-sidebar-content">
          ...
          {/* MINI PROFILE BAR (Klik untuk Full Page) */}
      ...
      {showFullProfile && (
        <div className="s-full-profile-overlay">
          ...
          <h1 style={{fontFamily: "'Playfair Display', serif", fontSize: '2.5rem', color: '#b8cbb8', marginBottom: '5px'}}>SANN404 FORUM</h1>
          <p style={{fontSize: '1rem', color: '#8e9b95', marginBottom: '20px', letterSpacing: '2px'}}>SANEXUSAI</p>
          
          <div className="s-collab-badge">
            Official Collaboration: IndoSawit.news x SanexusAI - Menciptakan ekosistem informasi cerdas.
          </div>

          <img src="https://e.top4top.io/p_3721610g20.jpg" alt="SANN404" style={{width: '120px', height: '120px', borderRadius: '50%', border: '3px solid #b8cbb8', objectFit: 'cover', marginBottom: '20px', boxShadow: '0 0 30px rgba(184,203,184,0.2)'}} />
          
          <h3 style={{fontSize: '1.5rem', color: '#ffffff', marginBottom: '5px'}}>SANN404</h3>
          ...
          <a href="https://whatsapp.com/channel/0029Vb6ukqnHQbS4mKP0j80L" target="_blank" rel="noreferrer" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', background: '#25D366', color: '#000', padding: '15px 30px', borderRadius: '50px', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 'bold', boxShadow: '0 10px 20px rgba(37,211,102,0.2)'}}>
      ...
      <main className="s-chat-area">
        ...
        {messages.map((msg, index) => (
          <div key={index}>
            {msg.role === 'user' ? (
              <div className="s-bubble-user animate-in fade-in slide-in-from-bottom-2">{msg.content}</div>
            ) : (
              // ANIMASI SMOOTH APPEAR DI SINI
